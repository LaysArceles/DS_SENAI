...
const UpdateProduct = () => {
    ...
    const getProductData = async () => {
        const response = await axios.get(`http://localhost:8080/Api/access/${_id}`)
        ...
        SetCat(response.data.response.category)
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            await axios.put(`http://localhost:8080/Api/access/${id}`, { name, description, price, stock, category, image })
                ...
                <main className="h-screen bg-gratient-to-t from-blue-200 to-blue-400 flex justify-center pt-20">
                    <div className="bg-white p-8 rounded-lg shadow-lg h-fit w-full max-w-md">
                        ...
                        <div className=" flex flex-col space-y-4">
                            ...
                            <div>
                                <input type="text" value={description} onChange={(e) => setDes(e.target.value)} placeholder="Update description"
                                    className="w-full border p-2 rounded focus:outline-blue-400" />
                            </div>
                            <div>
                                <input type="number" onChange={(p) => setPrice(Number(p.target.value))} placeholder="Uptade price"
                                    className="w-full border p-2 rounded focus:outline-blue-400" />
                            </div>
                            <div>
